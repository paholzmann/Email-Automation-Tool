...
if __name__ == "__main__":
    ...
    clean_df = pipeline.preprocessing(df=df)
    x_train, x_test, y_train, y_test = preprocessor.split_train_test_data(df=clean_df)
    y_pred = model.baseline_model(x_train=x_train["text_clean"], y_train=y_train, x_test=x_test["text_clean"])
    ...
    report.visualize_confusion_matrix(confusion_matrix=baseline_evaluation["confusion_matrix"])